...
complete -c roxide -n "__fish_roxide_needs_command" -s i -l interactive -d 'whether to prompt before removals' -r -f -a "{never\t'Never prompt',once\t'Prompt once before removing more than three files or when removing recursivly',always\t'Prompt before every removal',prompt-protected\t'Prompt only on write-protected files'}"
